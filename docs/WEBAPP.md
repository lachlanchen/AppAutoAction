...
AgInTi LabCanvas includes a bright-by-default local chat, artifact canvas, and preview workspace for 3D experiment design and paper figures. Use the top-bar theme button to switch to dark mode, and use the adjacent language dropdown to localize the interface.
...
- Target registry: the CLI still routes configured targets such as Blender, BioRender, Unity, and Unreal through `labcanvas.targets.json`.
...
The target dispatch box lists configured AgInTi LabCanvas targets and defaults to dry-run. Use it to test Blender, BioRender, Unity, Unreal, or custom targets from `labcanvas.targets.json`; each dispatch result is saved as a JSON artifact in the canvas.
